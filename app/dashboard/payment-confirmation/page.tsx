...
export default function PaymentConfirmation() {
  ...
  const handleConfirmPayment = async (paymentId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/payments/driver-confirm/${paymentId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleReportIssue = async (paymentId: string) => {
    ...
    try {
      ...
      // This would need to be a special API endpoint for drivers if this functionality is needed
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/payments/driver-report/${paymentId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
